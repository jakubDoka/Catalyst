...
impl RangeJoiner {
    ...
    pub fn exhaust(&mut self, range: IntRange) {
        ...
        let start_space = self.indices.binary_search_by(|e| e.start.cmp(&start))
            .into_ok_or_err();
        let end_space = self.indices.binary_search_by(|e| e.end.cmp(&end))
            .into_ok_or_err();
    ...
    pub fn missing_to(&self, buffer: &mut Vec<IntRange>) {
        ...
        if self.range.start != edge.start {
            buffer.push(IntRange::new(self.range.start..=edge.start - 1, self.range.bias));
        }

        buffer.extend(
            self.indices.windows(2)
                .map(|w| IntRange::new(w[0].end + 1..=w[1].start - 1, self.range.bias))
        );
        
        let edge = self.indices.last().unwrap(); 
        if self.range.end != edge.end {
            buffer.push(IntRange::new(edge.end + 1..=self.range.end, self.range.bias));
        }
    }
}