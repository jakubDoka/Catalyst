...
use lexer::{Source, Sources, SourcesExt, Span};

use crate::{ItemLexicon, Unit, Units};
    ...
    pub fn display(
        ...
        match self {
            Error::ScopeCollision { new, existing } => {
                ...
                new.underline_to(Palette::error().bold(), '^', sources, to, &|to| {
                    write!(to, "item with this identifier already exists")
                })?;
                existing.underline_to(Palette::info().bold(), '~', sources, to, &|to| {
