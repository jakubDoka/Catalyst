...
#[derive(Clone, Copy, Debug)]
pub struct IdentChainAst<'a> {
    pub first: Span,
    pub others: &'a [(Span, Span)],
}

impl<'a> Ast<'a> for IdentChainAst<'a> {
    type Args = ();

    const NAME: &'static str = "ident chain";

    fn parse_args_internal(ctx: &mut ParsingCtx<'_, 'a>, (): Self::Args) -> Result<Self, ()> {
        let first = ctx.advance().span;

        let mut others = bumpvec![];
        while ctx.at_tok(TokenKind::BackSlash) && ctx.at_next_tok(TokenKind::Ident) {
            others.push((ctx.state.current.span, ctx.state.next.span));
            ctx.advance();
            ctx.advance();
        }
        let others = ctx.ast_data.alloc_slice(&others);

        Ok(Self { first, others })
    }

    fn span(&self) -> Span {
        if let Some(&(_, last)) = self.others.last() {
            self.first.joined(last)
        } else {
            self.first
        }
    }
